...
myft <- flextable(mtcars, col_keys = c("am", "separator", "gear", "mpg", "drat" )) %>% 
  ...
  display(i = ~ drat > 3.6,  gear = fpar( ftext( gear, prop = fp_text(bold = TRUE, color="#C90000") ) ) ) %>%
  display( mpg = fpar(mpg, " and ", ftext( carb, prop = fp_text(bold = TRUE)), " carb(s)." )  ) %>%
  display( i = ~ gear < 4,
           drat = fpar( minibar( value = drat, max = max(.$drat), 
                                 barcol = "#C90000", bg = "transparent", 
                                 width = 1, height = .15) ) ) %>% 
  display( i = ~ gear < 4,
