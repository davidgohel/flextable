...
iris %>% 
  ...
  style(t_b(font.size = 12), part = "header") %>% 
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  style(c_b_b(), p_r(padding=2), part = "all" )
tabwid(myft)
```
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  style(i = ~ drat > 3.5, j = ~ drat, t_i(color="#44A530") )
tabwid(myft)
```
...
myft <- myft %>%
  display(
    ...
    gear = format_simple(format_integer(gear), pr_text_ = t_b()),
