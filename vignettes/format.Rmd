...
External images can be embed in tables. Below an illustration. The `fpar` call is containing a call to `external_img` which will use the R logo path.
...
if( file.exists(img.file) )
  myft <- myft  %>% 
    display(i = ~ Sepal.Length < 5, 
            Sepal.Length = fpar(external_img(img.file, width = .2, height = .2)) )
tabwid(myft)
```
