...
myft <- myft %>% 
  border( border.top = pr_border(color="orange"), 
          ...
          border.left = pr_border(color="white"), 
          border.right = pr_border(color="white"), 
          part = "all" )
...
ft <- flextable(head(mtcars, n = 10 )) %>% theme_vanilla() %>% 
  style( i = ~ drat > 3.5, j = ~ vs + am + gear + carb, 
        pr_t = pr_text(color="#C9C923", italic = TRUE) )
tabwid(ft)
```
...
```{r}
def_cell <- pr_cell(border = pr_border(color="#00C9C9"))
def_par <- pr_par(text.align = "center")
def_text <- pr_text(color="#999999", italic = TRUE)
