...
get_grid_data <- function(x, autowidths, wrapping) {
  ...
  grid_data$.part <- factor(
    grid_data$.part,
    levels = intersect(parts, grid_data$.part)
  )
  grid_data$.col_id <- factor(grid_data$.col_id, levels = x$col_keys)
  keycols <- c(".part", ".row_id", ".col_id")
  setorderv(grid_data, cols = keycols)
  grid_data[,
    c("row_index", "col_index") := list(
      rleid(.SD$.part, .SD$.row_id),
      as.integer(.SD$.col_id)
    )
  ]
  ...
  grid_data[,
    "cell_width" := safe_stat(.SD$col_width, FUN = sum, NA_value = NA_real_),
    by = "rowspan_group"
  ]
  grid_data[,
    "cell_height" := safe_stat(.SD$row_height, FUN = sum, NA_value = NA_real_),
    by = "colspan_group"
  ]
  grid_data[,
    "cell_min_height" := safe_stat(
      .SD$row_min_height,
      FUN = sum,
      NA_value = NA_real_
    ),
  ...
  grid_data[,
    "cell_max_height" := safe_stat_ext(
      .SD$row_max_height,
      FUN = sum,
      LENGTH = max(.SD$colspan),
      NA_value = NA_real_
    ),
    by = "colspan_group"
  ]
  grid_data[,
    "cell_height" := safe_stat(
      safe_stat(
        .SD$cell_height,
        .SD$cell_min_height,
        FUN = max,
  ...
  grid_data <- grid_data[
    grid_data$rowspan > 0 & grid_data$colspan > 0,
    ,
    drop = FALSE
  ]
...
grid_data_add_cell_info <- function(grid_data, x) {
  ...
  last_bottom_borders_data <- fortify_borders_data[,
    list(
  ...
  fortify_borders_data <- merge(
    fortify_borders_data,
    last_bottom_borders_data,
    by = c(".part", ".col_id", "vspan_id")
  )
  ...
  fortify_borders_data[
    fortify_borders_data$border.width.right ==
      shift(fortify_borders_data$border.width.left, type = "lead", fill = -1) &
      fortify_borders_data$.col_id != tail(x$col_keys, 1),
    c("border.width.right") := 0,
    by = c(".part", ".row_id")
  ]
...
grid_data_add_chunk_info <- function(grid_data, x, autowidths, wrapping) {
  ...
  # chunk types
  chunk_data[,
    "is_raster" := sapply(.SD$img_data, is.raster) |
      sapply(.SD$img_data, is.character)
  ]
  ...
  chunk_data[,
    "is_superscript" := .SD$is_text & .SD$vertical.align %in% "superscript"
  ]
  chunk_data[,
    "is_subscript" := .SD$is_text & .SD$vertical.align %in% "subscript"
  ]
  chunk_data[
  ...
  if (autowidths && !wrapping) {
    ...
    line_data <- part_data[,
      list(
        ...
        line_height = max(.SD$ascent, na.rm = TRUE) +
          max(.SD$descent, na.rm = TRUE)
      ),
    ...
    part_data[
      angle != 0,
      c("content_width", "content_height") := list(
        .SD$content_height,
        .SD$content_width
      )
    ]
  ...
  if (wrapping) {
    ...
    setorderv(
      word_data,
      cols = c(keycols, "chunk_index", "part_index", ".chunk_index")
    )
    word_data[,
      c("word_index", "word_count") := list(1:.N, .N),
      by = c(keycols, "chunk_index", "part_index")
    ]
    setorderv(
      word_data,
      cols = c(keycols, "chunk_index", "part_index", "word_index")
    )
    ...
    if (autowidths) {
      ...
      word_data[,
        "part_width" := fifelse(
          .SD$is_newline & .SD$part_index > 1 | .SD$is_space,
          0,
          .SD$width
        )
      ]
      ...
      word_data[,
        "content_width" := max(.SD$part_width, na.rm = TRUE),
        by = keycols
      ]
      word_data[,
        "content_height" := (sum(.SD$part_width, na.rm = TRUE) /
          .SD$content_width) *
          (max(.SD$part_ascent, na.rm = TRUE) +
            max(.SD$part_descent, na.rm = TRUE)),
        by = keycols
      ]
      word_data[
        angle != 0,
    ...
    char_data[,
      c("char_index", "char_count") := list(seq_len(.N), .N),
    ...
    part_data[,
      "children_index" := list(calc_grid_wrap_children(.SD)),
...
grid_data_adjust_widths <- function(grid_data) {
  ...
  if (nrow(dat_maxspan) > 0) {
    ...
    dat_maxspan[,
      c("prev_width", "content_min_width") := list(
        .SD$content_min_width,
        mapply(
          function(index, span, width) {
            sum(
              dat_minspan[
                col_index >= index & col_index < index + span
              ]$content_min_width
            )
          },
          .SD$col_index,
    ...
    if (any(dat_maxspan$diff > 0)) {
      dat_overflow <- dat_maxspan[
        diff > 0,
        .SD[which.max(.SD$diff), ],
        by = "col_index"
      ]
...
grid_data_adjust_heights <- function(grid_data) {
  ...
  if (nrow(dat_maxspan) > 0) {
    ...
    dat_maxspan[,
      c("prev_height", "content_min_height") := list(
        .SD$content_min_height,
        mapply(
          function(index, span, height) {
            sum(
              dat_minspan[
                row_index >= index & row_index < index + span
              ]$content_min_height
            )
          },
          .SD$row_index,
