...
iris %>% 
  ...
  set_style(t_b(), p_c(padding=4), part = "header") %>% 
  set_style(p_r(padding=4), part = "body") %>% 
  set_style(t_bi(), j = ~ Species ) %>% 
  set_border( border.top = pr_border(), border.bottom = pr_border(), part = "header" ) %>% 
  set_bg(i = ~ seq_along(Petal.Length) %% 2 == 0, color = "#EFEFEF") %>% 
  set_border( j = ~ break1 + break2, border.top = b_null(), border.bottom = b_null(), part = "all" ) %>% 
  set_bg(j = ~ break1 + break2, color = "transparent", part = "all") %>% 
  tabwid()
```
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  set_style(c_b_b(), p_r(padding=2), part = "all" )
tabwid(myft)
```
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  set_style(i = ~ drat > 3.5, j = ~ drat, t_i(color="#44A530") )
tabwid(myft)
```
...
myft <- myft %>%
  set_display(
    ...
    gear = format_simple(format_integer(gear), pr_text_ = p_l(padding.top=8)),
...
myft <- myft %>%
  set_display(
    mpg = format_that("{{ mpg_ }} mpg (# {{ cyl_ }} cyl.)", 
                      mpg_ = ftext( format_double(mpg, digits = 1), t_i() ), 
                      cyl_ = ftext( format_integer(cyl), t_i() )
    )
