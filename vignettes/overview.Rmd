...
webshot_ok <- require("webshot") && phantomjs_exists()
```
...
```{r  eval=webshot_ok, fig.asp=flextable_dim(ft)$aspect_ratio, fig.width=flextable_dim(ft)$width*3,out.width=flextable_dim(ft)$width*72}
plot(ft)
```
