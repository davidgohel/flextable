...
formatted as the result of an interpolation.

First the columns functions, they will define the formatting 
for each column. 

When working with a `regulartable`, user can have the control of the 
formatting function by using `set_formatter`. 

When working with a `flextable`, user can have the control of the 
formatting function by using `display`. The function enable the 
control of format at the cell level. It can also be used to mixed 
chunks of text with various text formats and images.
...
### define_text function

`flextable` content can be defined with function `define_text`.
...
Function `define_text` supports images insertion. Use function `as_image` in `as_paragraph` call:

```{r}
img.file <- file.path( R.home("doc"), "html", "logo.jpg" )

myft <- define_text( myft, i = ~ qsec > 18, j = "qsec", 
