...
iris %>% 
  ...
  set_style(t_b(font.size = 16), part = "header") %>% 
  set_style(pr_text(font.size = 16), part = "body") %>% 
  set_style(t_bi(font.size = 16), j = ~ Species ) %>% 
  padding(padding = 4, part = "header") %>% 
  border( border.top = pr_border(), part = "all" ) %>% 
  bg(i = ~ seq_along(Petal.Length) %% 2 == 0, color = "#EFEFEF") %>% 
  border( j = ~ break1 + break2, 
          border.top = b_null(), border.bottom = b_null(), part = "all" ) %>% 
  bg(j = ~ break1 + break2, color = "transparent", part = "all") %>% 
  autofit() %>% 
  tabwid()
```
