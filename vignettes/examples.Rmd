...
col_keys <- c("compound", "Df", "Sum Sq", "Mean Sq", "F value", "Pr(>F)") 
...
if( require("stats")){
  zz <- summary( aov(yield ~ block + N * P + K, npk) )
  summary_table <- as.data.frame(zz[[1]])
  summary_table$compound <- row.names(summary_table)
  summary_table$Df <- as.integer(summary_table$Df)
  summary_table <- summary_table[-nrow(summary_table), ]
  print(summary_table)
  
  ft <- regulartable( summary_table, col_keys = col_keys ) 
  ft <- theme_booktabs(ft)
  ft <- set_formatter( ft, `Pr(>F)` = stars_pval) 
  ft <- autofit(ft)
