...
iris %>% 
  ...
  style(pr_t = t_bold(font.size = 12), part = "header") %>% 
  style(pr_t = pr_text(font.size = 12), part = "body") %>% 
  style(pr_t = t_bi(font.size = 12), j = ~ Species ) %>% 
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  style(pr_c = c_b_b(), pr_p = p_right(padding=2), part = "all" )
tabwid(myft)
```
...
```{r warning=FALSE, message=FALSE}
myft <- myft %>% 
  style(i = ~ drat > 3.5, j = ~ drat, pr_t = t_italic(color="#44A530") )
tabwid(myft)
```
...
iris %>% group_by(Species) %>% do( head(., n = 3) ) %>%
  ...
  border(j=2, border = pr_border(width=0), part = "all") %>% 
  tabwid()
